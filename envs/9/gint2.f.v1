...
c      program gint
      ...
      real gp(10),unit(10),rp(10),rpara(10),gpara(10),dpa(2)
      ...
      open(unit=7,file='gpot2.para',status='old')
      open(unit=8,file='inc2.para',status='old')
...
cccc For initial data sets
      ...
      call writenbody(nj, xj, vj, time, tmgi, dpa)

      call calc_ext(nj,xj,a,gp)
      call ramr(nj,xj,vj,a,d,rp,gpara)

      if(rpara(4).gt.0.0) then
...
c-----  Output of position data
      ...
      subroutine writenbody(nj, xj, vj, time, tmgi, dpa)
      integer nj
      real xj(3, 2100000), vj(3, 2100000),dpa(2)
      real xj1(3, 2100000), vj1(3, 2100000)
      real time,tmgi
      integer i,k

      open(unit=12,file='tout.dat')

c      write(6,*) 'dpa',(dpa(k),k=1,2)
c-----For inclination of the output data

      pi=3.1415926535897
      theta1=2.*pi*dpa(1)/360.
      phai1=2.*pi*dpa(2)/360.
...
c------For disk inclination
      ...
      veli2=vj(2,i)
      veli3=vj(3,i)
      xj1(1,i)=cos(phai)*cos(theta)*posi1-sin(phai)*posi2
     &             -cos(phai)*sin(theta)*posi3
      xj1(2,i)=sin(phai)*cos(theta)*posi1+cos(phai)*posi2
     &             -sin(phai)*sin(theta)*posi3
      xj1(3,i)=sin(theta)*posi1+cos(theta)*posi3
      vj1(1,i)=cos(phai)*cos(theta)*veli1-sin(phai)*veli2
     &              -cos(phai)*sin(theta)*veli3
      vj1(2,i)=sin(phai)*cos(theta)*veli1+cos(phai)*veli2
     &             -sin(phai)*sin(theta)*veli3
      vj1(3,i)=sin(theta)*veli1+cos(theta)*veli3
      enddo

c*****Caution  xj1 and vj1 are only for output
      write(12,601) nj,time,tmgi
      do i=1,nj
      write(12,602)(xj1(k,i),k=1,3), (vj1(k,i),k=1,3)
      enddo
 601  format(i10,2(1pe13.5))
 602  format(6(1pe13.5))
c      write(12,*) nj,time,tmgi
c      do i=1,nj
c      write(12,*)(xj1(k,i),k=1,3), (vj1(k,i),k=1,3)
c      enddo
...
c-----   Calculations for ram pressure force 
      ...
      atr1_min=min(atr1_min,atr1)
      atr1_max=max(atr1_max,atr1)

c      write(6,*) 'rp',(rp(k),k=1,3)

c***** very important for rpt 
      vrt_max=100.0
      tiny=1.e-8
...
c-----For only disk (R<3.0) particles (not consider stripped gas)
c      r0=0.0
c      do k=1,3
c      r0=r0+xj(k,i)**2
c      enddo
...
c
c Caution: If vrt is large (>5), then RPS force become too large (unrealistic)
c   so, vrt should be kept in a reasonable range
c
c

      if(vrt.gt.vrt_max) vrt=vrt_max
      ...
      atr11=vrt*rp(4)*vrt*rp(k)/rpt/d(i)
      atr1=atr1+atr11**2
      enddo
      atr1=sqrt(atr1)
c      atr1_min=min(atr1_min,atr1)
c      if(atr1.gt.atr1_max) then
c      atr1_max=atr1
c      a11=vrt
c      a12=rp(1)
c      a13=rp(2)
...
c      write(6,*) 'atr min',atr1_min
c     write(6,*) 'atr max',atr1_max
c      write(6,*) 'a',a11,a12,a13,a14,a15,a16
c      write(6,*) 'a',a17,a18,s19
