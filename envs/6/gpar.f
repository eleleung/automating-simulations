...
c
c  For evolution of a particle around a galaxy (test particle method) 
c    by Kenji Bekki, 2018/1/12
c
c  For Eleanor & Mark
c  Program name:  gpar.f
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c /*
c#include <g5util.h>
...
c      program gint
      ...
      open(unit=7,file='gpot1.para',status='old')
      open(unit=8,file='orbit.para')

      read(1,*) endt,dt,imode,hmode

c*** For data output (Pos/Vel) at each time tep
      imode=1  
...
c-----  Only 1 particle calculation

      subroutine readnbody(nj, xj, vj, time)
      integer nj
      real xj(3, 2100000), vj(3, 2100000),pos(3),vel(3)
      integer i
      real time

      open(unit=11,file='gpar.para',status='old')
