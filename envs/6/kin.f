...
c          if(iw0(ij).eq.1.or.iw0(ij).eq.4) then
...
c             v1=tmp(j)*spn_final*v1
c             v2=tmp(j)*spn_final*v2
c             v3=tmp(j)*spn_final*v3
             v1=v1
             v2=v2
             v3=v3
...
c            if(iwas.eq.2) then
c            if(iwas.eq.1.or.iwas.eq.4) then
...
c            if(iwas.eq.2) then
            if(iwas.eq.1.or.iwas.eq.4) then
            id=2
            goto 115
            end if
...
c            write(10,*) nrbin*ntbin,rcut 
            ...
            vf(i,1,ij,ik)=vf(i,1,ij,ik)/real(nvf(1,ij,ik))
            vf2(i,1,ij,ik)=vf2(i,1,ij,ik)/real(nvf(1,ij,ik))
            vf2(i,1,ij,ik)=sqrt(vf2(i,1,ij,ik)-vf(i,1,ij,ik)**2)

c-----For vmax limitation 
      if(vf(i,1,ij,ik).gt.vmax_wo) then
      vf(i,1,ij,ik)=vmax_wo
      end if
      if(vf(i,1,ij,ik).lt.-vmax_wo) then
      vf(i,1,ij,ik)=-vmax_wo
      end if
            ...
            vf(i,2,ij,ik)=vf(i,2,ij,ik)/real(nvf(2,ij,ik))
            vf2(i,2,ij,ik)=vf2(i,2,ij,ik)/real(nvf(2,ij,ik))
            vf2(i,2,ij,ik)=sqrt(vf2(i,2,ij,ik)-vf(i,2,ij,ik)**2)
